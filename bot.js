...
class BasicBot {
  ...
  constructor(conversationState, userState, botConfig) {
    ...
    // Added me alundiak - EXPERIMENTAL - until encryption done.
    if (!luisConfig.appId){
      luisConfig.appId = process.env.luisAppID;
    }
    
    if (!luisConfig.authoringKey){
      luisConfig.authoringKey = process.env.luisSubscriptionKey || process.env.luisAuthoringKey;
    }    

    if (!luisConfig.region){
      luisConfig.region = process.env.luisRegion;
    }
    // Added me alundiak - EXPERIMENTAL - until encryption done.
    ...
    this.luisRecognizer = new LuisRecognizer({
      ...
      // azureRegion: luisConfig.region, // look like not needed when getEndpoint() but if used, then it overrides value from getEndpoint()
      // CAUTION: Its better to assign and use a subscription key instead of authoring key here.
      endpointKey: luisConfig.authoringKey,
      
      endpoint: luisConfig.getEndpoint() 
      // Need to ad if region is NOT "westus", because "westus" is default value in LUISRuntimeClient
      // https://github.com/Microsoft/Cognitive-LUIS-Node.js/blob/master/luis_sdk/index.js#L61
    });
